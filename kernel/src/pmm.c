...
static void *kalloc(size_t size) {
  ...
  if(ptr==NULL){
    ...
    pmm_lock(&(percpu[cpu].cpu_lock[bitsize]));
    ...
    pmm_spinlock_init(&(ptr->page_lock));
    pmm_unlock(&(percpu[cpu].cpu_lock[bitsize]));
  }
  else{
    while(ptr!=NULL){
  ...
  for(int i=0;i<ptr->max;i++){
    if(ptr->map[i]==0){
      pmm_lock(&(ptr->page_lock));
      ptr->map[i]=1;ptr->now++;
      pmm_unlock(&(ptr->page_lock));
